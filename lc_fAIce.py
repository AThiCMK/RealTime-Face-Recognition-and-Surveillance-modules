...
class FaceRecognitionApp(QtWidgets.QMainWindow):
    def __init__(self):
       ...
       # label = QtWidgets.QLabel("SURVEILLANCE AND TRACKING \n SYSTEM", self.central_widget)
        #label.setAlignment(QtCore.Qt.AlignCenter)  # Align text to center
        #label.setStyleSheet("color: white; font-size: 55px; font-family: Galvanizer; font-weight: bold;")  # Set text color, font size, family, and weight
        #label.adjustSize()
        #label.move(60, 150)  # Center the label horizontally
        #label.adjustSize()
        ...
        button_layout.addWidget(self.sort_images_button)

        self.layout.addStretch(1)
        self.layout.addLayout(button_layout)
        
        self.layout.addSpacerItem(QtWidgets.QSpacerItem(20, 60, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding))
...
class rtspStreamApp(QtWidgets.QWidget):
    ...
    def start_live_stream_rtsp(self, urls):
        for url in urls:
            video_capture = cv2.VideoCapture(f"{url}")
