...
class FaceRecognitionApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        if not os.path.exists(os.path.join(self.BASE_DIR, "trespassers")):
            os.mkdir(self.trespassers_folder)
        self.all_face_encodings = []
        self.all_face_names = []
        self.face_data_file = os.path.join(self.BASE_DIR, r"utils\face_data.pkl")
        ...
        background_image = QtGui.QPixmap(os.path.join(self.BASE_DIR, r"static\bg1.jpg"))
        ...
        self.add_class_button = self.create_circular_button("Add or Update Users", os.path.join(self.BASE_DIR, r"static\add.webp"), self.show_add_class_window)
        self.start_stream_button = self.create_circular_button("Start Live Stream", os.path.join(self.BASE_DIR, r"static\live.png"), self.start_live_stream_window)
        self.sort_images_button = self.create_circular_button("Sort Images", os.path.join(self.BASE_DIR, r"static\rr.png"), self.show_sort_images_window)
    ...
    def setup_menu_bar(self):
        ...
        kebab_button.setIcon(QtGui.QIcon(os.path.join(self.BASE_DIR, r"static\kb.png")))
    ...
    def show_sort_images_window(self):
        ...
        sort_images_window.setWindowIcon(QtGui.QIcon(os.path.join(self.BASE_DIR, r"static\rr.png")))
    ...
    def show_add_class_window(self):
        ...
        add_class_window.setWindowIcon(QtGui.QIcon(os.path.join(self.BASE_DIR, r"static\add.webp")))
...
class StartLiveStreamDialog(QtWidgets.QDialog):
    def __init__(self):
        ...
        self.BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        self.setWindowIcon(QtGui.QIcon(os.path.join(self.BASE_DIR, r"static\live.png")))
    ...
    def create_url_entry_widgets(self, num_urls):
        ...
        for i in range(num_urls):
            ip_label = QtWidgets.QLabel(f"RTSP IP {i+1}:")
...
class webStreamApp(QtWidgets.QWidget):
    def __init__(self, all_face_encodings, all_face_names, webcam_port=None):
        ...
        self.live_stream_label.setFixedSize(640, 480)
        self.live_stream_label.setSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        ...
        self.alert_triggered = {class_name: False for class_name in self.all_face_names}
    ...
    def start_live_stream_webcam(self, port):
        self.video_capture = thread.ThreadingClass(port)
        if port is not None:
            ...
            with open(csv_file_path, mode='a', newline='') as csv_file:
                ...
                while self.streaming:
                    ...
                    if frame is not None:
                        ...
                        middle_line_y = frame_height // 2 +100
                        ...
                        for (top, right, bottom, left), name in zip(face_locations, names):
                            ...
                            if top < middle_line_y < bottom:
                                if name == "Unknown":
                                    with self.alert_lock:
                                        if not self.unknown_alert_triggered:
                                            self.unknown_alert_triggered = True
                                            self.report_trespasser(frame, (top, right, bottom, left))
                                            threading.Thread(target=self.sound_alert).start()
                                else:
                                    with self.alert_lock:
                                        if not self.alert_triggered[name]:
                                            self.alert_triggered[name] = True
                                            threading.Thread(target=self.sound_alert).start()
                            
                        qt_pixmap = self.convert_cv_qt(frame)
    ...
    def sound_alert(self):
        try:
            if platform.system() == 'Windows':
                ...
                subprocess.run(['beep', '-f', '1000', '-l', '500'])  # Beep at 1000 Hz for 500 ms
        finally:
            pass
        
    def report_trespasser(self, frame, location):
        top, right, bottom, left = location
        trespasser_image = frame[top:bottom, left:right]
        timestamp = time.strftime("%Y-%m-%d_%H:%M:%S")
        trespasser_filename = os.path.join(self.trespassers_folder, f"trespasser_{timestamp}.png")
        cv2.imwrite(trespasser_filename, trespasser_image)
        print(f"Trespasser reported: {trespasser_filename}")

        # Display trespasser image with timestamp on the side of the live_stream_window
        pixmap = self.convert_cv_qt(trespasser_image)
        trespasser_label = QtWidgets.QLabel()
        trespasser_label.setPixmap(pixmap)
        trespasser_label.setAlignment(QtCore.Qt.AlignCenter)
        timestamp_label = QtWidgets.QLabel(timestamp)
        timestamp_label.setAlignment(QtCore.Qt.AlignCenter)

        QTimer.singleShot(0, lambda: self.update_ui_with_trespasser(trespasser_label, timestamp_label))
    ...
    def update_ui_with_trespasser(self, trespasser_label, timestamp_label):
        layout = QtWidgets.QVBoxLayout()
        layout.addWidget(trespasser_label)
...
class rtspStreamApp(QtWidgets.QWidget):
    def __init__(self, all_face_encodings, all_face_names, rtsp_urls=None):  # Updated arguments
        ...
        self.live_stream_window = QtWidgets.QMainWindow()
        self.live_stream_window.setWindowTitle("Live Stream")
        self.live_stream_window.setWindowIcon(QtGui.QIcon(os.path.join(self.BASE_DIR, r"static\live.png")))
    ...
    def cleanup(self):
        self.streaming = False
        if self.video_captures is not None:
