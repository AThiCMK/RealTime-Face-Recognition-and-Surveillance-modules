    ...
    def add_new_class(self, class_name, add_class_window):
        ...
        while True:
            ...
            if key == ord('c'):
                ...
                video_capture.release()  # Release video capture resources
                cv2.destroyAllWindows()  # Close OpenCV windows
                break
        self.update_face_data(class_name)
        add_class_window.hide()  # Close only the add_class_window dialog
...
class LiveStreamApp(QtWidgets.QWidget):
    def __init__(self, all_face_encodings, all_face_names, rtsp_url=None, webcam_port=None):
        ...
        # Connect closeEvent to close method
        self.live_stream_window.closeEvent = self.closeEvent
        # Add instance variables for tracking last detection time for each class
        self.last_detection_time = {class_name: None for class_name in self.all_face_names}
 
        
    def closeEvent(self, event):
        print("Closing live stream window...")
    ...
    def cleanup(self):
        ...
        if self.video_capture is not None and self.video_capture.isOpened():
            self.video_capture.release()  # Release camera resources if opened
        cv2.destroyAllWindows()
